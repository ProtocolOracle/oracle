"use client";

import {
  getBlockHeight,
  getBones
} from "@/lib/ethereum";
import { use, useEffect, useState } from "react";
import {
  Card,
  CardContent,
  CardDescription,
  CardHeader,
  CardTitle,
} from "@/components/ui/card";

const OverviewBox = () => {
  const [blockHeight, setBlockHeight] = useState(0);
  const [bones, setBones] = useState(0);
  const [totalSupply, setTotalSupply] = useState(0);
  useEffect(() => {
    const fetchBlockHeight = async () => {
      const height = await getBlockHeight();
      setBlockHeight(height);
    };

    fetchBlockHeight();
  }, []);
  useEffect(() => {
    const fetchBones = async () => {
      const bones = await getBones();
      setBones(bones);
    };
    fetchBones();
  }, []);
  // useEffect(() => {
  //   const fetchSupply = async () => {
  //     const supply = await getSupply();
  //     setTotalSupply(supply);
  //   };
  //   fetchSupply();
  // }, []);
  return (
    <div className="p-8 grid gap-4 md:grid-cols-2 lg:grid-cols-4">
      <Card className="bg-transition-colors backdrop-blur text-white">
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          <CardTitle className="text-sm font-medium">Block Height</CardTitle>
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 24 24"
            fill="none"
            stroke="currentColor"
            strokeLinecap="round"
            strokeLinejoin="round"
            strokeWidth="2"
            className="h-4 w-4 text-muted-foreground"
          >
            <path d="M12 2v20M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6" />
          </svg>
        </CardHeader>
        <CardContent>
          <div className="text-2xl font-bold">{blockHeight}</div>
        </CardContent>
      </Card>
      <Card className="bg-transition-colors backdrop-blur text-white">
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          <CardTitle className="text-sm font-medium">Total Supply</CardTitle>
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 24 24"
            fill="none"
            stroke="currentColor"
            strokeLinecap="round"
            strokeLinejoin="round"
            strokeWidth="2"
            className="h-4 w-4 text-muted-foreground"
          >
            <path d="M22 12h-4l-3 9L9 3l-3 9H2" />
          </svg>
        </CardHeader>
        <CardContent>
          <div className="text-2xl font-bold">{totalSupply}</div>
        </CardContent>
      </Card>
      <Card className="bg-transition-colors backdrop-blur text-white">
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          <CardTitle className="text-sm font-medium">Deployed Bones</CardTitle>
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 24 24"
            fill="none"
            stroke="currentColor"
            strokeLinecap="round"
            strokeLinejoin="round"
            strokeWidth="2"
            className="h-4 w-4 text-muted-foreground"
          >
            <rect width="20" height="14" x="2" y="5" rx="2" />
            <path d="M2 10h20" />
          </svg>
        </CardHeader>
        <CardContent>
          <div className="text-2xl font-bold">{bones}</div>
        </CardContent>
      </Card>
      <Card className="bg-transition-colors backdrop-blur text-white">
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          <CardTitle className="text-sm font-medium">Process</CardTitle>
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 24 24"
            fill="none"
            stroke="currentColor"
            strokeLinecap="round"
            strokeLinejoin="round"
            strokeWidth="2"
            className="h-4 w-4 text-muted-foreground"
          >
            <path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2" />
            <circle cx="9" cy="7" r="4" />
            <path d="M22 21v-2a4 4 0 0 0-3-3.87M16 3.13a4 4 0 0 1 0 7.75" />
          </svg>
        </CardHeader>
        <CardContent>
          <div className="text-2xl font-bold">
            {(bones / totalSupply) * 100} %
          </div>
        </CardContent>
      </Card>
    </div>
  );
};

export default OverviewBox;
function getTransactions() {
  throw new Error("Function not implemented.");
}
